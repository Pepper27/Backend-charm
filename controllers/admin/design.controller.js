...
module.exports.getDesigns = async (req, res) => {
  try {
    ...
    const userIds = [...new Set(pageItems.map((d) => (d?.userId ? String(d.userId) : "")).filter(Boolean))]
      .filter((v) => mongoose.Types.ObjectId.isValid(v))
      .map((v) => new mongoose.Types.ObjectId(v));
    const users = userIds.length
      ? await AccountClient.find({ _id: { $in: userIds }, deleted: false }).select("fullName email").lean()
      : [];
...
module.exports.getDesignById = async (req, res) => {
  try {
    ...
    if (String(id).startsWith("bundle:")) {
      ...
      const bundle = (cart?.bundles || []).find((b) => String(b.bundleId) === String(bundleId)) || null;
      if (!cart || !bundle) {
        return res.status(404).json({ message: "Không tìm thấy design" });
      }

      // Do not expose anonymous guest bundle designs in admin.
      const userObjId = cart?.userId && mongoose.Types.ObjectId.isValid(String(cart.userId))

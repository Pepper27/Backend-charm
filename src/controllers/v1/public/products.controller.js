...
module.exports.getBestSellers = async (req, res) => {
  try {
          ...
          rows: [
            ...
            { $limit: limit },
            {
              $project: {
                ...
                priceMax: 1,
                totalSold: 1,
                thumbnail: { $arrayElemAt: ["$variants.images", 0] },
              },
            },
          ],
    ...
    return v1.ok(res, rows, { meta: { page, limit, total, totalPages } });
  } catch (error) {
  ...
  try {
    ...
    if (collectionId) {
      if (mongoose.Types.ObjectId.isValid(collectionId)) {
        collectionIds = [new mongoose.Types.ObjectId(collectionId)];
      } else {
        // If not a valid ObjectId, try to find by slug or name
        const collection = await Collection.findOne({
          deleted: false,
          $or: [{ slug: collectionId }, { name: collectionId }],
        })
          .select("_id")
          .lean();
        if (collection?._id) {
    ...
    if (match.priceMax || match.priceMin) {
      ...
      match.$or = [
        {
          priceMin: { $exists: true, $ne: null, $ne: 0 },
          priceMax: { $exists: true, $ne: null, $ne: 0 },
        },
          ...
          rows: [
            ...
            { $limit: limit },
            {
              $project: {
                ...
                collections: {
                  $cond: [
                    { $isArray: "$collectionsDocs" },
                    {
                      $map: {
                        input: "$collectionsDocs",
                        as: "c",
                        in: {
                          _id: "$$c._id",
                          name: "$$c.name",
    ...
    if (includeFilters) {
      ...
      if (priceRangesCsv.length) {
        ...
        if (priceOr.length)
          facetMatch.$or = facetMatch.$or ? facetMatch.$or.concat(priceOr) : priceOr;
      }

      // inStock filter
      if (inStockCsv.length) {
        const stockOr = [];
        if (inStockCsv.includes("in_stock"))
          stockOr.push({ $expr: { $gt: [{ $sum: "$variants.quantity" }, 0] } });
        if (inStockCsv.includes("out_of_stock"))
          stockOr.push({ $expr: { $lte: [{ $sum: "$variants.quantity" }, 0] } });
        if (stockOr.length)
          facetMatch.$or = facetMatch.$or ? facetMatch.$or.concat(stockOr) : stockOr;
      }
      ...
      const collections = (allCollections || []).map((c) => ({
        _id: c._id,
        name: c.name,
        count: collMap.get(String(c._id)) || 0,
      }));

  ...
  try {
    ...
    if (collectionId) {
      ...
      const parts = raw
        ...
        .map((v) => v.trim())
        .filter(Boolean);
      const ids = (parts.length ? parts : [raw]).filter(Boolean);

      const invalid = ids.find((value) => !mongoose.Types.ObjectId.isValid(value));
      if (invalid) {
        return res.status(400).json({ message: "collectionId không hợp lệ" });
      }

      // Match products that belong to ANY of the selected collections.
      find.collections = { $in: ids.map((v) => new mongoose.Types.ObjectId(v)) };
    }
    ...
    const hasMaterialFilter =
      material !== undefined && material !== null && String(material).trim() !== "";
...
module.exports.createProduct = async (req, res) => {
  try {
    ...
    const collections = parseJsonField(req.body.collections, undefined);
    ...
    for (let i = 0; i < variants.length; i++) {
      ...
      const code =
        helper.generateCodeVariant(material, v.color || "", v.size || "") +
...
module.exports.updateProduct = async (req, res) => {
  try {
    ...
    if (collections !== undefined) {
      ...
      } else if (collections === null || collections === "") {
        product.collections = [];
      }
    }
    const materialsFromVariants = [

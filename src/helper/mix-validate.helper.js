...
const validateAndPrice = async ({ bracelet, items }) => {
  ...
  for (let i = 0; i < safeItems.length; i++) {
    ...
    if (Number(charmVariant.quantity) <= 0) {
      validationErrors.push(
        buildError(`items[${i}]`, "Charm variant out of stock", {
          charmVariantCode: it.charmVariantCode,
        })
      );
    }
  ...
  if (Number(braceletVariant.quantity) <= 0) {
    validationErrors.push(
      buildError("bracelet.variantCode", "Bracelet variant out of stock", { braceletVariantCode })
    );
  }
